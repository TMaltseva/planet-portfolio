...
export default function SceneContainer({ children }: SceneContainerProps) {
  ...
  useFrame((state) => {
    ...
    if (!isRotating) {
      // Автоматическое медленное вращение когда не взаимодействуют мышью
      if (Math.abs(rotationVelocity) < 0.001) {
        sceneRef.current.rotation.y += 0.0008; // Очень медленное вращение
      } else {
        // Продолжение инерции после взаимодействия
        sceneRef.current.rotation.y += rotationVelocity;
        setRotationVelocity(prev => prev * 0.95);
      }
    }
